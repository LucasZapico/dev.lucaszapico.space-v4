...
#### PSQL with Postgres URL

```sh 
psql postgres://<pg_user>:<pg_password>@<host>:<port>/<db_name>
```

```sh 
psql postgres[ql]://[username[:password]@][host[:port],]/database[?parameter_list]
```
...
[More on postgres connection url - prisma.io](https://www.prisma.io/dataguide/postgresql/short-guides/connection-uris)
