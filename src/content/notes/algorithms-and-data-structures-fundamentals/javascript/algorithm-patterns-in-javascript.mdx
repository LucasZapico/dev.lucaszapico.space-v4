...
Sets are similar to arrays except there are no duplicate elements.

```javascript
function mySet() {
  // the var collection withhold the set
  var collection = []

  // this method will check for the presence of an element and return a true or false
